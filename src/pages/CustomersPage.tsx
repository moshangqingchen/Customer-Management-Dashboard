...
export function CustomersPage({
  ...
  const [showVipOnly, setShowVipOnly] = useState(vipOnly);
  const [menu, setMenu] = useState<{ x: number; y: number; customer: Customer } | null>(null);
  const filtered = useMemo(() => customers
    .filter((customer) => !showVipOnly || customer.vipLevel > 0)
    .filter((customer) => JSON.stringify(customer).toLowerCase().includes(query.toLowerCase())), [customers, query, showVipOnly]);
  ...
  useEffect(() => setShowVipOnly(vipOnly), [vipOnly]);
  ...
  return (
    <div className="page-content">
      <PageHeader
        eyebrow="客户档案"
        title="客户管理"
        description="统一管理跨平台身份、联系方式、地址与历史订单；已标星客户可随时单独查看。"
        actions={<Button onClick={onNew}><Plus size={17} />新建客户</Button>}
      />
      <div className="toolbar">
        <label className="search-field"><Search size={17} /><input aria-label="搜索客户" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索姓名、电话、微信或平台网名…" /></label>
        <div className="quick-filters" role="group" aria-label="客户视图">
          <button type="button" className={!showVipOnly ? "active" : ""} aria-pressed={!showVipOnly} onClick={() => setShowVipOnly(false)}>全部客户</button>
          <button type="button" className={showVipOnly ? "active" : ""} aria-pressed={showVipOnly} onClick={() => setShowVipOnly(true)}><Star size={13} />星级 VIP</button>
        </div>
        <span className="result-count">{filtered.length} 位客户</span>
      </div>
      {filtered.length === 0 ? <EmptyState icon={showVipOnly ? <Star size={28} /> : <ContactRound size={28} />} title={showVipOnly ? "还没有星级 VIP" : "还没有客户"} description={showVipOnly ? "在客户档案中点亮星级后，会自动出现在这里。" : "先创建客户，再为客户录入订单。"} action={<Button onClick={onNew}><Plus size={16} />新建客户</Button>} /> :
      ...
      <div className="customer-list-panel">{filtered.map((customer) => <article
        ...
        onContextMenu={(event) => {
          event.preventDefault();
          onSelect?.(customer);
          setMenu({ ...clampMenuPosition(event.clientX, event.clientY), customer });
        }}
      >
        <div className="customer-card-top"><div className="avatar">{customer.name.slice(0, 2)}</div><div className="customer-title"><h3>{customer.name}</h3><StarRating value={customer.vipLevel} compact /></div><button type="button" className="icon-button" aria-label={`修改客户 ${customer.name}`} onClick={(event) => { event.stopPropagation(); onEdit(customer); }}><Edit3 size={16} /></button></div>

...
export function SettingsPage({ settings, onChanged }: { settings: AppSettings; onChanged: () => void }) {
  ...
  const runBackup = async () => {
    ...
    setMessage("");
    try {
      const fallbackDir = `${settings.libraryRoot ?? ""}\\_数据库备份`;
      const path = await api.runBackup(settings.backupDir ?? fallbackDir);
      setMessage(`备份已完成：${path}`);
      onChanged();
    } catch (reason) {
      setMessage(`备份失败：${String(reason)}`);
    } finally {
  ...
  return (
    <div className="page-content">
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card settings-card-actions"><div className="settings-icon purple"><DatabaseBackup size={22} /></div><div><h2>数据库自动备份</h2><p>每天启动时自动备份一次，最多保留最近 30 份。也可以手动立即生成一份备份。</p><code>{settings.backupDir ?? "默认保存在 Windows 文档目录"}</code></div><div className="settings-action-stack"><Button variant="secondary" onClick={() => choose("backup")}>设置备份目录</Button><Button onClick={runBackup} disabled={backupRunning}>{backupRunning ? "备份中…" : "立即备份"}</Button></div></section>
